...
class CommonChecker:
    ...
    smooth_swing_keys = ['highwsmooth', 'wpercentsmooth', 'circlesmooth', 'humvolumemin', 'peakwmax',
                         'relativevolumemin']
    ...
    def check_smooth_swing(self, data: dict, param: str) -> Tuple[str, str, Sequence[str], Sequence[str]]:
        """
        checks if smooth swing parameters are correct, warning for unreal movement parameters
        :param data: dict with data
        ...
        if error or swing is None:
            return error, "", [], []
        wrong_data_keys = []
        warning, wrong_keys = check_keys(swing, self.smooth_swing_keys)
        w = check_number(swing, 'highwsmooth', 0, self.w_high)
        if w:
            wrong_data_keys.append('highwsmooth')
            warning += w
        w = check_number(swing, 'wpercentsmooth', 0, 100)
        if w:
            wrong_data_keys.append('wpercentsmooth')
            warning += w
        w = check_number(swing, 'circlesmooth', 10, 9999)
        if w:
            wrong_data_keys.append('circlesmooth')
            warning += w
        w = check_number(swing, 'peakwmax', 1, self.w_high)
        if w:
            wrong_data_keys.append('peakwmax')
            warning += w
        w = check_number(swing, 'humvolumemin', 0, 100)
        if w:
            wrong_data_keys.append('humvolumemin')
            warning += w
        w = check_number(swing, 'relativevolumemin', 0, 100)
        if w:
            wrong_data_keys.append('relativevolumemin')
            warning += w
        return "", warning, wrong_data_keys, wrong_keys

    def check_swing(self, data: dict, param: str) -> Tuple[str, str, Sequence[str], Sequence[str]]:
        """
        checks if swing parameters are correct, warning for unreal movement parameters
        :param data: dict with data
        :param param: key to find
        :return: error and warning or empty strings
        """
