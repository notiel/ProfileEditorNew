...
defaults: Dict[str, Any] = {'Blade': {'BandNumber': 1, 'PixPerBand': 144, 'StartFlashFrom': 15},
                            ...
                            'Motion':
                                {'SwingSimple': {'HighW': 8, 'WPercent': 50, 'Circle': 640, 'CircleW': 6},
                                 'SwingSmooth': {'HighWSmooth': 0.1, 'WPercentSmooth': 1, 'CircleSmooth': 640,
                                                 'HumVolumeMin': 30, 'PeakWMax': 6, 'RelativeVolumeMin': 30},
                                 'Spin': {'Enabled': 1, 'Counter': 4, 'W': 9.4, 'Circle': 640, 'WLow': 6.3},
                                 'Clash': {'HighA': 3500, 'Length': 15, 'HitLevel': -200, 'LowW': 2.65},
                                 'Stab': {'Enabled': 1, 'HighA': 15, 'LowW': 1.73, 'HitLevel': -200, 'Length': 30,
                                          'Percent': 90},
...
motion_keys = ['SwingSimple', 'SwingSmooth', 'Spin', 'Clash', 'Stab', 'Screw']
swing_keys = ['HighW', 'WPercent', 'Circle', 'CircleW']
swing_smooth_keys = ['HighWSmooth', 'WPercentSmooth', 'CircleSmooth', 'HumVolumeMin', 'PeakWMax', 'RelativeVolumeMin']
...
class CommonData:
    ...
    @staticmethod
    def check_section(new_data: dict, check_function: Callable, param: str, required: bool,
    ...
    def load_data_from_text(self, text: str):
        ...
        try:
            ...
            warning += w
            w, new_data[motion] = self.check_section(new_data[motion], checker.check_swing, 'SwingSimple', True,
                                                     defaults[motion])
            warning += w
            w, new_data[motion] = self.check_section(new_data[motion], checker.check_smooth_swing, 'SwingSmooth', True,
                                                     defaults[motion])
            warning += w
